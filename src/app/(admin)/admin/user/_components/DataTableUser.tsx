      ...
      {selectedRowsCount > 0 && (
        <div className="bg-emerald-50 border border-emerald-200 rounded-2xl px-5 py-3.5 flex items-center justify-between shadow-sm animate-in slide-in-from-top-2">
          <span className="text-emerald-800 font-bold text-sm">
            {selectedRowsCount} user{selectedRowsCount > 1 ? "s" : ""} selected
          </span>
      ...
      <div className="hidden lg:grid grid-cols-12 gap-4 px-6 py-3 bg-gray-50/80 border border-gray-100 rounded-2xl mb-1 sticky top-0 z-10 backdrop-blur-md">
        {table.getHeaderGroups().map((headerGroup) =>
          headerGroup.headers.map((header) => {
            ...
            else if (header.id === "phoneNumber") colSpanClass = "col-span-2";
            else if (header.id === "createdAt") colSpanClass = "col-span-2";
            else if (header.id === "isVerifyByAdmin") colSpanClass = "col-span-1";
      ...
      <div className="flex flex-col gap-3 relative min-h-[400px]">
        {isLoading ? (
          ...
          table.getRowModel().rows.map((row) => (
            ...
            >
              ...
              {row.getVisibleCells().map((cell) => {
                ...
                else if (cell.column.id === "phoneNumber") colSpanClass = "col-span-2";
                else if (cell.column.id === "createdAt") colSpanClass = "col-span-2";
                else if (cell.column.id === "isVerifyByAdmin") colSpanClass = "col-span-1";
                ...
                return (
                  ...
                  >
                    {/* Mobile Label */}
                    <span className="lg:hidden text-xs font-bold text-gray-400 uppercase w-24 flex-shrink-0">
          ...
          <div className="bg-white border border-gray-100 border-dashed rounded-3xl p-12 flex flex-col items-center justify-center text-center shadow-sm">
            ...
            <p className="text-gray-500 text-sm max-w-sm">We couldn&apos;t find any users matching your current filters. Try adjusting your search criteria.</p>
          </div>
        )}
      </div>
      ...
      {!isLoading && (
        <div className="mt-4 bg-white border border-gray-100 rounded-2xl p-4 shadow-sm flex items-center justify-between">
          <div className="text-sm font-medium text-gray-500">
            Showing <span className="font-bold text-gray-900">{items?.length || 0}</span> users
          </div>
