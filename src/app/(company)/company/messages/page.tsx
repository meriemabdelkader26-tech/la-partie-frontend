...
export default function MessagesPage() {
  ...
  const sendMutation = useMutation({
    ...
    onSuccess: (data: any) => {
      ...
      // If we started a new conversation, select it
  ...
  // Handle initial userId or conversationId from URL
  useEffect(() => {
      ...
      <div className="flex items-center justify-between px-2 shrink-0">
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
          ...
          <h1 className="text-3xl md:text-4xl font-black text-black tracking-tighter">
            Influencer <span className="text-gray-400">Chats</span> 💬
          </h1>
        </motion.div>
      </div>
        ...
        <Card className="flex-1 bg-white border-black/5 rounded-[3rem] shadow-large overflow-hidden flex flex-col relative min-w-0">
          ...
          {selectedConversation ? (
            <>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Live Messenger</span>
                    </div>
                  </div>
                </div>
              ...
              {/* Messages Area */}
              <div className="flex-1 overflow-y-auto p-6 md:p-10 space-y-6 custom-scrollbar bg-[url('/chat-bg.png')] bg-repeat">
             ...
             <div className="flex-1 flex flex-col items-center justify-center p-10 text-center space-y-8">
                <div className="p-10 bg-black text-white rounded-[3rem] shadow-2xl relative">
                  <Plus className="size-16" />
                  <div className="absolute -top-2 -right-2 size-8 bg-emerald-500 rounded-full border-4 border-white animate-pulse" />
                </div>
                <div className="space-y-2">
                   <h3 className="text-3xl font-black text-black tracking-tighter">Start New Chat</h3>
                   <p className="text-gray-500 font-medium max-w-sm mx-auto leading-relaxed">
                     You are about to start a new conversation with this talent. Send your first pitch!
                   </p>
                </div>
                <div className="w-full max-w-md bg-white p-2 rounded-3xl border border-black/5 shadow-large flex items-center gap-2">
                   <Input 
                      placeholder="Pitch your campaign idea..." 
                      value={messageInput}
                      onChange={(e) => setMessageInput(e.target.value)}
                      onKeyDown={(e) => e.key === 'Enter' && handleSend()}
                   ...
                   <Button 
                    onClick={handleSend}
                    disabled={!messageInput.trim() || sendMutation.isPending}
                    className="size-14 rounded-2xl bg-black text-white shrink-0"
                   >
                    <Send className="size-6" />
                   </Button>
                </div>
